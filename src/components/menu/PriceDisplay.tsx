  ...
  if (specialRippchen || specialSchnitzel) {
    ...
    return (
      <>
        ...
        <div className="text-lg text-red-600 font-bold animate-pulse">{formatPriceWithCurrency(item.price)}</div>
      </>
    );
  }
  return <div className="text-lg font-bold text-gray-900">{formatPriceWithCurrency(item.price)}</div>;
};
